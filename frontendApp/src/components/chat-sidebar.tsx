...
import { Bot, ChevronLeft, ChevronRight, LogOut, MessageSquare, PenSquare, Search, Trash2, X } from "lucide-react";
			...
			<aside className="flex h-full flex-col border-r"
				...
				<div className="flex justify-center px-3 pt-5 pb-2">
					<button type="button" onClick={() => setCollapsed(false)}
						className="flex h-9 w-9 items-center justify-center" style={{ color: "#766f62" }}
						aria-label="Expand sidebar">
						<ChevronRight className="h-4 w-4" />
					</button>
				</div>
				<div className="flex justify-center px-3 pb-4">
					<button type="button" onClick={() => { setCollapsed(false); setShowAgents(true); }}
						className="flex h-9 w-9 items-center justify-center" style={{ color: "#2a7a44" }}
						aria-label="New chat">
				...
				<div className="flex flex-1 flex-col items-center gap-1 overflow-y-auto px-3 py-2">
					{sessions.slice(0, 12).map((s) => (
						<button key={s.id} type="button" onClick={() => onSelectSession(s)}
		...
		<aside className="flex h-full flex-col border-r"
			...
			<div className="flex items-center justify-between px-5 py-5">
				...
				<button type="button" onClick={() => setCollapsed(true)}
					className="flex h-8 w-8 items-center justify-center" style={{ color: "#766f62" }}
			...
			<div className="px-3 pb-3">
				<button type="button" onClick={() => setShowAgents(!showAgents)}
					...
					onMouseLeave={(e) => { if (!showAgents) (e.currentTarget as HTMLButtonElement).style.background = "transparent"; }}>
			...
			{showAgents && (
				<div className="border-t border-b px-3 pb-3" style={{ borderColor: "#e8e4d8" }}>
					<div className="flex items-center gap-2 pt-3 pb-2">
						...
						/>
						{search && (
							<button type="button" onClick={() => setSearch("")}><X className="h-3.5 w-3.5" style={{ color: "#a09a8a" }} /></button>
						)}
					</div>
					<div className="max-h-52 overflow-y-auto">
						{filteredAgents.length === 0 ? (
							<p className="py-3 text-center text-xs" style={{ color: "#a09a8a" }}>Aucun agent trouvé</p>
						) : filteredAgents.map(agent => (
							<button key={agent.id} type="button" onClick={() => handleSelectAgent(agent)}
								...
								<div className="min-w-0 flex-1">
									...
									{agent.description && <p className="truncate text-xs" style={{ color: "#a09a8a" }}>{agent.description}</p>}
								</div>
							</button>
						))}
					</div>
			...
			<div className="flex-1 overflow-y-auto">
				{groupedSessions.length === 0 ? (
					<div className="flex flex-col items-center justify-center gap-3 px-5 py-16 text-center">
						...
						<p className="text-sm" style={{ color: "#766f62" }}>Aucune conversation</p>
					</div>
					...
					<div key={group.label} className="mb-2">
						...
						{group.sessions.map((session) => (
							<button key={session.id} type="button"
								...
								onMouseEnter={(e) => { if (activeSessionId !== session.id) (e.currentTarget as HTMLButtonElement).style.background = "#ebe8e0"; }}
								onMouseLeave={(e) => { if (activeSessionId !== session.id) (e.currentTarget as HTMLButtonElement).style.background = "transparent"; }}>
			...
			<div className="border-t px-3 py-3" style={{ borderColor: "#e8e4d8" }}>
				<div className="flex items-center gap-3 px-2 py-2">
					<div className="flex h-8 w-8 shrink-0 items-center justify-center text-xs font-medium"
					...
					<button type="button" onClick={onSignOut}
						className="shrink-0 p-1.5 transition-colors" style={{ color: "#766f62" }}
						aria-label="Sign out"
						onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.color = "#c0392b"; }}
						onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.color = "#766f62"; }}>
						<LogOut className="h-4 w-4" />
