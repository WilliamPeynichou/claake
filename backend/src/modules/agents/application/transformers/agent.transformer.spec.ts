...
describe("AgentTransformer", () => {
	it("transforms entity to DTO with correct field mapping", () => {
		const entity = new AgentEntity(
			"id-1", "My Agent", "my-agent", "Description", "Long desc",
			"coding", ["ai", "ml"], ["claude-sonnet-4-20250514"], "CLOUD",
			"https://config.url", "https://image.url", ["https://ss1.png"],
			"FREE", 0, 1, "APPROVED", null, 100, 4.5, 10,
			"creator-1", "John Doe",
	...
	it("lowercases enum values", () => {
		const entity = new AgentEntity(
			"id", "n", "s", "d", null, "cat", [], [],
			"HYBRID", null, null, [], "ONE_TIME", 9.99, 1, "PENDING",
			null, 0, 0, 0, "u", null, new Date(), new Date(),
		);
