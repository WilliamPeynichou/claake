/*
  ...
  - The values [PUBLISHED] on the enum `AgentStatus` will be removed. If these variants are still used in the database, this will fail.
  - You are about to drop the column `average_rating` on the `agents` table. All the data in the column will be lost.
  - You are about to drop the column `downloads_count` on the `agents` table. All the data in the column will be lost.
  - You are about to drop the column `model` on the `agents` table. All the data in the column will be lost.
  - You are about to drop the column `price_type` on the `agents` table. All the data in the column will be lost.
  - You are about to drop the column `reviews_count` on the `agents` table. All the data in the column will be lost.
  - You are about to drop the column `sandbox_uses` on the `agents` table. All the data in the column will be lost.
  - You are about to drop the column `version` on the `agents` table. All the data in the column will be lost.
  - You are about to alter the column `price` on the `agents` table. The data in that column could be lost. The data in that column will be cast from `DoublePrecision` to `Decimal(10,2)`.
...
-- AlterEnum
BEGIN;
CREATE TYPE "AgentStatus_new" AS ENUM ('DRAFT', 'PENDING', 'APPROVED', 'REJECTED', 'SUSPENDED');
...
-- AlterTable
ALTER TABLE "agents" DROP COLUMN "average_rating",
DROP COLUMN "downloads_count",
DROP COLUMN "model",
DROP COLUMN "price_type",
DROP COLUMN "reviews_count",
DROP COLUMN "sandbox_uses",
DROP COLUMN "version",
ADD COLUMN     "config_url" TEXT,
ADD COLUMN     "credit_cost" INTEGER NOT NULL DEFAULT 1,
ADD COLUMN     "download_count" INTEGER NOT NULL DEFAULT 0,
ADD COLUMN     "embedding" vector(1536),
ADD COLUMN     "models" TEXT[] DEFAULT ARRAY['claude-sonnet-4-20250514']::TEXT[],
ADD COLUMN     "permissions" JSONB,
ADD COLUMN     "pricing_model" "PricingModel" NOT NULL DEFAULT 'FREE',
...
-- AlterTable
ALTER TABLE "categories" ADD COLUMN     "agent_count" INTEGER NOT NULL DEFAULT 0;

-- AlterTable
ALTER TABLE "users" DROP COLUMN "full_name",
ADD COLUMN     "api_keys_encrypted" JSONB,
